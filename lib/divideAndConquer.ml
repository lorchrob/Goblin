...
= fun ast -> 
  ...
  | element :: elements -> (match element with 
    | ProdRule (nt, rhss, p) ->
      ...
      ProdRule (nt, rhss, p) :: ast', subproblems1 @ subproblems2
...
= fun ast -> 
  ...
  let asts = List.map (fun ast -> match ast with 
    | ProdRule (sym, _, _) :: _-> TopologicalSort.dead_rule_removal ast sym 
    | TypeAnnotation (sym, ges, scs, p) :: _ -> Some [TypeAnnotation (sym, ges, scs, p)]
