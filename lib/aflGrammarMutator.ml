...
let create_nonterm_map (ast : ast) : (string, prod_rule_rhs list) Hashtbl.t =
  ...
  let rec traverse_ast = function
    ...
    | ProdRule (name, rhs_list) :: rest ->
        ...
        traverse_ast rest
    | TypeAnnotation (_, _, _) :: rest -> traverse_ast rest
  in
  traverse_ast ast;
  nonterm_map
...
let rule_based_mutation (ast : ast) (nonterm_map : (string, prod_rule_rhs list) Hashtbl.t) : ast =
  let rec mutate_rule = function
    ...
    | TypeAnnotation (name, typ, sc) -> TypeAnnotation (name, typ, sc)
  and mutate_rhs = function
    | Rhs (elements, scList) -> 
        Rhs (List.map (function
          | Nonterminal nt -> 
              (match Hashtbl.find_opt nonterm_map nt with
               | Some alternatives -> 
                   (match List.nth_opt alternatives (Random.int (List.length alternatives)) with
                    | Some (Rhs (_, _)) -> Nonterminal nt
                    | _ -> Nonterminal nt)
               | None -> Nonterminal nt)
          | element -> element) elements, scList)
...
let random_mutation (ast : ast) : ast =
  let randomize element = 
    match element with
    | Nonterminal nt -> if Random.bool () then Nonterminal nt else StubbedNonterminal (nt, "random")
    | NamedNonterminal (name, nt) -> if Random.bool () then NamedNonterminal (name, nt) else Nonterminal nt
    | other -> other
  in
  let mutate_rule = function
    | ProdRule (name, rhs_list) -> 
        ProdRule (name, List.map (function
          | Rhs (elements, scList) -> Rhs (List.map randomize elements, scList)
          | StubbedRhs s -> StubbedRhs s) rhs_list)
    | other -> other
  in
  List.map mutate_rule ast

let recursive_mutation (ast : ast) (max_depth : int) : ast =
  let expand element depth = 
    match element with
  ...
  let mutate_rhs rhs depth = 
    match rhs with
    | Rhs (elements, scList) -> Rhs (List.map (fun e -> expand e depth) elements, scList)
    | StubbedRhs s -> StubbedRhs s
  in
  let mutate_rule rule depth =
    match rule with
...
let splicing_mutation (ast1 : ast) (ast2 : ast) : ast =
  let splice_elements elems1 elems2 = 
    let combine e1 e2 =
      match e1, e2 with
      | Nonterminal n1, Nonterminal n2 -> Nonterminal (if Random.bool () then n1 else n2)
      | NamedNonterminal (name, n1), NamedNonterminal (_, n2) -> NamedNonterminal (name, if Random.bool () then n1 else n2)
      | x, _ -> x
    in
    List.map2 combine elems1 elems2
  in
  let splice_rules r1 r2 =
    match r1, r2 with
    | ProdRule (name1, rhs_list1), ProdRule (_, rhs_list2) -> 
        ProdRule (name1, List.map2 (fun rhs1 rhs2 -> 
          match rhs1, rhs2 with
          | Rhs (elems1, scList1), Rhs (elems2, _) -> Rhs (splice_elements elems1 elems2, scList1)
          | StubbedRhs s, _ -> StubbedRhs s
          | _, StubbedRhs s -> StubbedRhs s) rhs_list1 rhs_list2)
    | other, _ -> other
  in
...
let subtree_trimming (ast : ast) : ast =
  let trim_rhs = function
    | Rhs (elements, scList) -> Rhs (List.filter (fun _ -> Random.bool ()) elements, scList)
    | other -> other
  in
  let trim_rule = function
    | ProdRule (name, rhs_list) -> ProdRule (name, List.map trim_rhs rhs_list)
    | other -> other
  in
  List.map trim_rule ast

let rec recursive_trimming (ast : ast) (depth : int) : ast =
...
let mutate_ast (ast : ast) : ast =
  ...
  | 3 -> if Random.bool () then splicing_mutation ast (List.rev ast) else splicing_mutation ast ast
